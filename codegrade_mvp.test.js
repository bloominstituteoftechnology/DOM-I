...
describe("text contents", () => {
  ...
  test("[4] main content paragraphs contain the correct text", () => {
    expect(
      screen.getByText(/Features content elementum magna eros/, {
        selector: ".top-content .text-content p",
      })
    );
    expect(
      screen.getByText(/About content elementum magna eros/, {
        selector: ".top-content .text-content p",
      })
    );
  });
  ...
  test("[6] bottom content headings contain the correct text", () => {
    expect(
      screen.getByText(/Services content elementum magna eros/, {
        selector: ".bottom-content .text-content p",
      })
    );
    expect(
      screen.getByText(/Product content elementum magna eros/, {
        selector: ".bottom-content .text-content p",
      })
    );
    expect(
      screen.getByText(/Vision content elementum magna eros/, {
        selector: ".bottom-content .text-content p",
      })
    );
  });
  test("[7] contact information elements contain the correct text", () => {
    expect(screen.getByText("Contact", { selector: "section.contact h4" }));
    expect(
      screen.getByText("123 Way 456 Street Somewhere, USA", {
        selector: "section.contact p",
      })
    );
    expect(
      screen.getByText("1 (888) 888-8888", { selector: "section.contact p" })
    );
    expect(
      screen.getByText("sales@greatidea.io", { selector: "section.contact p" })
    );
  });
  test("[8] copyright information link contains the correct text", () => {
    expect(screen.getByText(/Copyright Great Idea/, { selector: "footer a" }));
  });
});
describe("img sources", () => {
  ...
  test("[10] call to action img has the correct src", () => {
    expect(screen.getByAltText("Image of a code snippet")).toHaveAttribute(
      "src",
      "http://localhost:9000/img/cta.png"
    );
  });
  test("[11] middle img has the correct src", () => {
    expect(screen.getByAltText(/Image of code snippets/)).toHaveAttribute(
      "src",
...
describe("class names", () => {
  ...
  test("[13] no other links have a class name of italic", () => {
    expect(document.querySelector("footer a")).not.toHaveAttribute(
      "class",
      "italic"
    );
  });
